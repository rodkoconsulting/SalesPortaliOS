...
struct DbOperation {
    ...
    static func databaseInit() throws -> String? {
        ...
        let sqlInvPo = "CREATE TABLE IF NOT EXISTS INV_PO (ITEM_CODE STRING, PO_NO STRING, ON_PO REAL, PO_ETA STRING, PO_DATE STRING, PO_CMT STRING, PRIMARY KEY (ITEM_CODE, PO_NO))"
        ...
        let sqlOrderListHeader = "CREATE TABLE IF NOT EXISTS ORDER_LIST_HEADER(ORDER_NO STRING, ORDER_DATE STRING, SHIP_DATE STRING, ARR_DATE STRING, DIVISION_NO CHARACTER(2), CUSTOMER_NO STRING, STATUS STRING, HOLD STRING, COOP STRING, COMMENT STRING, SHIP_TO STRING, PRIMARY KEY(ORDER_NO))"
        ...
        let sqlSampleListHeader = "CREATE TABLE IF NOT EXISTS SAMPLE_LIST_HEADER(ORDER_NO STRING, SHIP_DATE STRING, REP STRING, SHIP_TO STRING, IS_POSTED INT, PRIMARY KEY(ORDER_NO))"
        ...
        let sqlOrderHeader = "CREATE TABLE IF NOT EXISTS ORDER_HEADER(ORDER_NO INTEGER PRIMARY KEY AUTOINCREMENT, TYPE STRING, DIVISION_NO CHARACTER(2), CUSTOMER_NO STRING, SAVE_TIME STRING, SHIP_DATE CHARACTER(6), NOTES STRING, COOP_QTY INT, COOP_NO STRING, TOTAL_QTY REAL, TOTAL_PRICE REAL, PO_NO STRING, SHIP_TO CHARACTER(4))"
        ...
        let sqlAccountAddresses = "CREATE TABLE IF NOT EXISTS ACCOUNT_ADDRESSES(DIVISION_NO CHARACTER(2), CUSTOMER_NO STRING, CODE CHARACTER(4), NAME STRING, ADDRESS STRING, PRIMARY KEY(DIVISION_NO, CUSTOMER_NO, CODE))"
        let sqlHolidays = "CREATE TABLE IF NOT EXISTS HOLIDAYS_DATES (DATE STRING PRIMARY KEY)"
        let insLastSyncHolidays = "INSERT INTO LAST_SYNC (table_name) SELECT 'HOLIDAYS' WHERE NOT EXISTS(SELECT 1 FROM LAST_SYNC WHERE table_name='HOLIDAYS')"
        let insLastSyncHolidaysDates = "INSERT INTO LAST_SYNC (table_name) SELECT 'HOLIDAYS_DATES' WHERE NOT EXISTS(SELECT 1 FROM LAST_SYNC WHERE table_name='HOLIDAYS_DATES')"
        ...
        let insLastSyncAccountAddresses = "INSERT INTO LAST_SYNC (table_name) SELECT 'ACCOUNT_ADDRESSES' WHERE NOT EXISTS(SELECT 1 FROM LAST_SYNC WHERE table_name='ACCOUNT_ADDRESSES')"
        ...
        guard dB.executeStatements(sqlLastSync) &&
            ...
            dB.executeStatements(sqlAccountAddresses) &&
            ...
            dB.executeStatements(insLastSyncAccountAddresses) &&
            ...
            dB.executeStatements(insLastSyncSamples) &&
            dB.executeStatements(sqlHolidays) &&
            dB.executeStatements(insLastSyncHolidays) &&
            dB.executeStatements(insLastSyncHolidaysDates) else {
                dB.close()
                throw ErrorCode.dbError
        }
