    ...
    func queryDb() -> (gridData:NSMutableArray?, searchData: [[String : String]]?, isManager: Bool) {
        ...
        if dB.open() {
            let sqlQuery = "WITH INVOICES AS (" +
                ...
                "UNION " +
                "SELECT ITEM_CODE, DESC, BRAND, MASTER_VENDOR, VINTAGE, UOM, SIZE, DAMAGED_NOTES, CLOSURE, TYPE, VARIETAL, ORGANIC, BIODYNAMIC, FOCUS, COUNTRY, REGION, APPELLATION, RESTRICT_OFFSALE, RESTRICT_OFFSALE_NOTES, RESTRICT_PREMISE, RESTRICT_ALLOCATED, RESTRICT_APPROVAL, RESTRICT_MAX, RESTRICT_STATE, RESTRICT_SAMPLE, RESTRICT_BO, RESTRICT_MO, UPC, SCORE_WA, SCORE_WS, SCORE_IWC, SCORE_BH, SCORE_VM, SCORE_OTHER, RECEIPT_DATE, REGEN, NAT, VEGAN, HVE FROM ACCOUNTS_ITEMS_INACTIVE " +
                "WHERE ITEM_CODE IN (SELECT DISTINCT ITEM_CODE FROM INVOICES)" +
                ") " +
                "SELECT DISTINCT d.ITEM_CODE, i.QUANTITY AS 'LAST_QTY', i.INVOICE_DATE AS 'LAST_DATE', i.INVOICE_NO AS 'LAST_INVOICE', i.Price as 'LAST_PRICE', QTY_AVAIL, QTY_OH, ON_SO, ON_MO, ON_BO, DESC, BRAND, MASTER_VENDOR, VINTAGE, UOM, SIZE, DAMAGED_NOTES, CLOSURE, TYPE, VARIETAL, ORGANIC, BIODYNAMIC, FOCUS, COUNTRY, REGION, APPELLATION, RESTRICT_OFFSALE, RESTRICT_OFFSALE_NOTES, RESTRICT_PREMISE, RESTRICT_ALLOCATED, RESTRICT_APPROVAL, RESTRICT_MAX, RESTRICT_STATE, RESTRICT_SAMPLE, RESTRICT_BO, RESTRICT_MO, UPC, SCORE_WA, SCORE_WS, SCORE_IWC, SCORE_BH, SCORE_VM, SCORE_OTHER, RECEIPT_DATE, REGEN, NAT, VEGAN, HVE, PRICE_DESC FROM ITEMS d LEFT OUTER JOIN  INV_QTY q ON q.ITEM_CODE = d.ITEM_CODE LEFT OUTER JOIN INV_PRICE p ON p.ITEM_CODE = d.ITEM_CODE LEFT OUTER JOIN INVOICES i ON i.ITEM_CODE = d.ITEM_CODE WHERE (p.PRICE_LEVEL = '" + self.account.priceLevel.rawValue + "' OR p.PRICE_LEVEL ISNULL) AND (p.DATE ISNULL or p.DATE = (SELECT DATE FROM INV_PRICE AS p2 WHERE p2.ITEM_CODE = p.ITEM_CODE and p2.PRICE_LEVEL = p.PRICE_LEVEL AND p2.DATE <= '" + self.date + "' ORDER BY DATE DESC LIMIT 1)) AND (i.INVOICE_DATE ISNULL OR i.INVOICE_DATE = (SELECT INVOICE_DATE FROM INVOICES AS i2 WHERE i2.ITEM_CODE = i.ITEM_CODE ORDER BY INVOICE_DATE DESC LIMIT 1)) AND (i.INVOICE_NO ISNULL OR i.INVOICE_NO = (SELECT INVOICE_NO FROM INVOICES AS i3 WHERE i3.ITEM_CODE = i.ITEM_CODE ORDER BY INVOICE_DATE DESC, INVOICE_NO DESC LIMIT 1)) ORDER BY BRAND, DESC"
